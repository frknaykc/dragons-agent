        ...
        async respond(request) {
          if (request.task === "Run the advanced advisory preference workflow.") {
            ...
            if (request.toolOutputs.length === 0) {
              ...
              assert.deepEqual(request.tools.map((tool) => tool.name), ["read_fixture", "mcp__fixture__inspect", "plan_list", "plan_add", "plan_update", "plan_set_status", "plan_remove", "suggest_memory", "delegate_subagent", "delegate_parallel_subagents"]);
