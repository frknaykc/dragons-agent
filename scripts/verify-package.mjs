...
try {
  ...
  for (const forbidden of [".env", "/src/", ".test.js", "MILESTONES.md", ".hermes/", "acceptance-", "provider-acceptance", "live-smoke", "stream-trace"]) assert.equal(contents.includes(forbidden), false, `package contains forbidden ${forbidden}`);
